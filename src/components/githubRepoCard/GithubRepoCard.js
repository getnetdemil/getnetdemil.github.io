        ...
        <div key={repo.id} onClick={() => openRepoinNewTab(repo.url)}>
          <div className="repo-name-div">
            ...
            <p className="repo-name" style={{ color: theme.text }}>
              {repo.name || repo.nameWithOwner?.split('/')[1] || repo.nameWithOwner}
            </p>
          </div>
          ...
          <div className="repo-details">
            ...
            >
              {repo.createdAt ? `Created on ${repo.createdAt.split("T")[0]}` : `⭐ ${repo.stargazers?.totalCount || 0} stars`}
            </p>
            {repo.primaryLanguage && (
              <div className="repo-language">
                <span className="language-color" style={{ backgroundColor: repo.primaryLanguage.color }}></span>
                <span style={{ color: theme.secondaryText }}>{repo.primaryLanguage.name}</span>
              </div>
            )}
          </div>
